...
@SpringBootTest
public class AdmissionCommiteeTests {
	@Autowired
	...
	public void testUpdateAdmissionCommiteeMember() {
		...
		assertEquals(member, service.updateCommiteeMember(member));
	}
	...
	public void testViewCommiteeMember() {
		Mockito.when(repo.findByadminId(1)).thenReturn(new AdmissionCommiteeMember(1, "admin1", "9900990099"));
		assertEquals(1, service.viewCommiteeMember(1).getAdminId());
	}

	@Test
	public void testViewAllCommiteeMember() {
		Mockito.when(repo.findAll()).thenReturn(Stream.of(new AdmissionCommiteeMember(1, "admin1", "9900990099"),
				new AdmissionCommiteeMember(2, "admin2", "9999900000")).collect(Collectors.toList()));
